...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrmmax, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrmmax( 3 ), 'function', 'returns expected value' );
	t.end();
});

tape( 'the accumulator function computes a moving maximum incrementally', function test( t ) {
	var expected;
	var actual;
	var data;
	var acc;
	...
	data = [ 2.0, 3.0, 2.0, 4.0, 3.0, 4.0, 2.0, 2.0, 2.0, 1.0 ];
	N = data.length;
	...
	expected = [ 2.0, 3.0, 3.0, 4.0, 4.0, 4.0, 4.0, 4.0, 2.0, 2.0 ];
...
tape( 'the accumulator function correctly handles signed zeros', function test( t ) {
	...
	for ( i = 0; i < data.length; i++ ) {
		...
		if ( isPositiveZero( expected[ i ] ) ) {
			t.strictEqual( isPositiveZero( v ), true, 'returns expected value for window '+i );
		} else {
			t.strictEqual( isNegativeZero( v ), true, 'returns expected value for window '+i );
		}
	}
	t.end();
});
